...
const removeCountFromCart = async ({ username }) => {
  console.log(`CartService | removeCountFromCart`);
  try {
    let response = await axios.patch(
      `${ApiContants.BACKEND_API.BASE_API_URL}${ApiContants.BACKEND_API.CART}/${username}/remove-count`,
      {},
      {
        headers: authHeader(getToken()),
      }
    ...
    if (response?.status === 200) {
      return {
        status: true,
        message: `Count removed from cart successfully`,
        data: response?.data?.data,
      };
    } else {
      return {
        status: false,
        message: `Count removal from cart failed`,
      };
    }
  } catch (error) {
    console.log(error?.response);
...
export default { getCartItems, addToCart, removeFromCart, removeCountFromCart };
