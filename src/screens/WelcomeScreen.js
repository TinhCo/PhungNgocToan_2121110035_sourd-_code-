...
const WelcomeScreen = ({ navigation }) => {
  ...
  const onViewRef = useRef(({ changed }) => {
    ...
    setCurrentPage(changed[0].index);
  });
  ...
  const handleNext = () => {
    if (currentPage < General.WELCOME_CONTENTS.length - 1) {
      setWelcomeListIndex(currentPage + 1);
  ...
  const handleSkip = () => {
    setWelcomeListIndex(General.WELCOME_CONTENTS.length - 1);
    setCurrentPage(General.WELCOME_CONTENTS.length - 1);
    welcomeList.current.scrollToIndex({
      index: General.WELCOME_CONTENTS.length - 1,
    });
  };
