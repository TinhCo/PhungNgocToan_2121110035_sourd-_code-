...
const SignupScreen = ({ navigation }) => {
  ...
  const signInWithFacebook = async () => {
    try {
      console.log("Bắt đầu quá trình đăng nhập Facebook...");
      setIsLoading(true);

      await signInWithRedirect(auth, provider);

      // Kiểm tra kết quả sau khi chuyển hướng
      await getRedirectResult(auth);

      // Nếu không có lỗi, chuyển hướng đến trang HomeTabs
    } catch (error) {
  ...
  const handleRedirectResult = async () => {
    try {
      ...
      if (result.user) {
        ...
        // Chuyển hướng đến màn hình HomeTabs
        isLoading("HomeScreen");
      } else if (result.credential) {
        console.log("Thông tin đăng nhập từ Facebook:", result.credential);
      } else {
