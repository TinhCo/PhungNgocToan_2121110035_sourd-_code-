...
const HomeScreen = ({ navigation }) => {
  ...
  const [isHorizontalView, setIsHorizontalView] = useState(true); // Thêm state này
  const [searchQuery, setSearchQuery] = useState("");
  const [searchResults, setSearchResults] = useState([]);

  const handleSearch = () => {
    // Gọi API hoặc thực hiện tìm kiếm trong mảng `restaurants` tại đây
      ...
      <ScrollView style={styles.listContainer}>
        <View style={styles.horizontalListContainer}>
          ...
          {isHorizontalView ? (
            ...
            // Dùng FlatList với chế độ hiển thị dọc tương tự
            <FlatList
              data={restaurants}
              keyExtractor={(item) => item?.id}
              renderItem={({ item }) => (
