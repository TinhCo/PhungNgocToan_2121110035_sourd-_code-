...
import { placeOrderSuccess } from "../actions/orderActions";
...
const CartScreen = ({ navigation }) => {
  ...
  const handleCheckout = async () => {
    try {
      // Lặp qua từng mục trong giỏ hàng và xóa số lượng
      for (const item of [...cart.cartItems]) {
        const response = await CartService.removeCountFromCart({
          username: item.username,
        });

        // Kiểm tra response và xử lý phản hồi từ server
        if (response.status) {
      ...
      // Cuối cùng, điều hướng đến trang Order hoặc thực hiện các thao tác khác
          ...
          <ScrollView>
            ...
            <View style={styles.promoCodeContainer}>
              <View style={styles.rowAndCenter}>
                <Entypo name="ticket" size={25} color={Colors.DEFAULT_YELLOW} />
